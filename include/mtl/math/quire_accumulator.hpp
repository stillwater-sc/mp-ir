#pragma once
// mp-ir -- accumulator_traits bridge to Universal's quire super-accumulators.
//
// A quire is a fixed-size super-accumulator that captures sums of products of
// a fixed-size arithmetic type EXACTLY, deferring rounding to a single
// conversion at the end. The concept applies to any fixed-size arithmetic --
// integer, fixpnt, posit, lns, cfloat -- not to posits specifically; posit is
// simply the first number system wired up here.
//
...
// can accumulate inner products in an exact quire and round once -- exactly
// what an iterative-refinement residual r = b - A x wants when the residual is
// meant to carry more information than the working precision holds.
//
// This header lives in mp-ir, NOT in MTL5: MTL5 is the general linear-algebra
...
// the specialization sits next to the generic accumulator_traits it extends.
// No opt-in macro is needed -- if you are building against sw::mp_ir, the
// composition is what you asked for.
//
...
//   auto rho = mtl::dot<Quire>(r, z);   // accumulate rho in the quire, round once
...
/// binds to that default -- if your project overrides `bt`, extend the pattern
